...
def evaluate_vit(model, template, search):
    ...
    with torch.no_grad():
        # overall
        for i in range(T_w):
            _ = model(template, search,training=False)
        start = time.time()
        for i in range(T_t):
            _ = model(template, search,training=False)
        torch.cuda.synchronize()
...
def evaluate_prompt(model, template, search,prompt_bbox):
    '''Speed Test'''
    macs1, params1 = profile(model, inputs=(template, search,prompt_bbox),
                             custom_ops=None, verbose=False)
    macs, params = clever_format([macs1, params1], "%.3f")
    print('overall macs is ', macs)
    print('overall params is ', params)
    ...
    with torch.no_grad():
        # overall
        for i in range(T_w):
            _ = model(template, search,prompt_bbox=prompt_bbox,training=False)
        start = time.time()
        for i in range(T_t):
            _ = model(template, search,prompt_bbox=prompt_bbox,training=False)
        torch.cuda.synchronize()
...
if __name__ == "__main__":
    ...
    script='ostrack'
    config='prompt_reptoken_weight_384_3data_v4-log-5'
    '''update cfg'''
    yaml_fname = os.path.join(prj_path,'experiments/%s/%s.yaml' % (script, config))
    config_module = importlib.import_module('lib.config.%s.config' % script)
    ...
    if cfg.TEST.USE_PROMPT:
        model_constructor = model_module.build_ostrack_traj
    else:
    ...
    print(config)
    if cfg.TEST.USE_PROMPT:
        prompt_bbox = torch.randn(bs, cfg.PROMPT.TRACKLEN, 4)
        prompt_bbox = prompt_bbox.to(device)
        evaluate_prompt(model, template, search,prompt_bbox)
    else:
