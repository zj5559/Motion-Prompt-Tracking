...
class Lasot(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, vid_ids=None, split=None, data_fraction=None):
        ...
        self.trackdata_path = os.path.join(env_settings().pretrained_networks,'..','traj_data/lasot')
    ...
    def _read_trackdata(self,seq_id):
        ...
        file=os.path.join(self.trackdata_path,seq_name+'.txt')
        gt=pandas.read_csv(file, delimiter=',', header=None, dtype=np.float32, na_filter=False, low_memory=False).values
        gt=gt[:,:4]
        img_tmp=self._get_frame(self._get_sequence_path(seq_id),0)
        h,w,_=img_tmp.shape
        gt[:,0]*=w
        gt[:, 1] *= h
        gt[:, 2] *= w
            ...
            if key=='track_bbox' and traj:
                ...
                for f_id in frame_ids:
                    if f_id + sparse < len(value) and random.random() < 0.5:
                        ...
                        gt_traj=anno['bbox'][f_id + sparse:min(len(value),f_id + 1 + sparse * tracklen):sparse].clone()
